...
namespace FreeSql.Generator
{
  class ConsoleApp
  {
    ...
    string ArgsSnowflake { get; }// <--------------------------------------------------------------------【RUNGROO】
    bool ArgsValidate { get; }// <--------------------------------------------------------------------【RUNGROO】
    string ArgsBaseType { get; set; }
    string ArgsPrimaryKeyType { get; set; }
    ...
    public ConsoleApp(string[] args, ManualResetEvent wait)
    {
      ...
      if (args[0] == "?" || args0 == "--help" || args0 == "-help")
      {
        ...
        * {11}(达梦数据库)、{12}(人大金仓数据库)、{13}(神舟通用数据库)
        ...
        -Snowflake                【Rung】雪花算法的特性（Attribute）类名
        -Validate                 【Rung】是否添加验证特性（Attribute）,基于 ASP.NET Core 内置验证特性（System.ComponentModel.DataAnnotations）
        -BaseType                 【Rung】基类类型
        -PrimaryKeyType           【Rung】强制设置主键类型
        {15}
      ...
      for (int a = 0; a < args.Length; a++)
      {
        switch (args[a].Trim().ToLower())
        {
          ...
          case "-validate":// <--------------------------------------------------------------------【RUNG】
            ...
            break;
          case "-basetype":// <--------------------------------------------------------------------【RUNG】
            ArgsBaseType = args[a + 1];
            a++;
            break;
          case "-primarykeytype":// <--------------------------------------------------------------------【RUNG】
            ArgsPrimaryKeyType = args[a + 1];
            a++;
            break;
          default:
      ...
      .Build())
      {
        ...
        if (tables.Any() == false) tables = fsql.DbFirst.GetTablesByDatabase();
        ...
        foreach (var table in outputTables)
        {
          ...
          if (ArgsDbType == DataType.Sqlite)
          {
            if (table.Name.StartsWith("sqlite_")) continue;
          }

          var sw = new StringWriter();
          var model = new RazorModel(fsql, ArgsNameSpace, ArgsSnowflake, ArgsValidate, ArgsBaseType, ArgsPrimaryKeyType, ArgsNameOptions, tables, table);
      ...
      if (ArgsReadKey) Console.ReadKey();
      wait.Set();
    }
  }
}
