...
{
  public RazorModel(IFreeSql fsql, string nameSpace, string snowflake, bool validate, string baseType, string primaryKeyType, bool[] NameOptions, List<DbTableInfo> tables, DbTableInfo table)
  {
    this.fsql = fsql;
  ...
  public string Snowflake { get; set; }// <--------------------------------------------------------------------【RUNG】

  public bool Validate { get; set; }// <--------------------------------------------------------------------【RUNG】

  public string BaseType { get; set; }// <--------------------------------------------------------------------【RUNG】

  public string PrimaryKeyType { get; set; }// <--------------------------------------------------------------------【RUNG】
  ...
  {
    if (col.IsPrimary && !string.IsNullOrEmpty(this.PrimaryKeyType))
    {
    ...
    if (fsql.Ado.DataType == DataType.Sqlite && col.DbTypeText == "INTEGER")
    {
      return "long";
    }
  ...
  public string GetColumnAttribute(DbColumnInfo col, bool isInsertValueSql = false)
  {
    ...
    if (sb.Any() == false) return null;
    result = "[Column(" + string.Join(", ", sb) + ")]";

    List<string> ignoreColumns = new List<string> { "id", "app_id", "create_time", "create_user_id", "create_user_name", "is_deleted" };

    if (this.Validate && ignoreColumns.Contains(col.Name) == false)
    {
      sb.Clear();

      if (col.IsNullable == false)
      {
        sb.Add($"Required(ErrorMessage = \"[{this.GetCsName(col.Name)}]不能为空\")");
      }
      if (col.CsType == typeof(string))
      {
        sb.Add($"StringLength({col.MaxLength}, ErrorMessage = \"长度不能大于{col.MaxLength}个字符。\")");
      }
...
Unknow{1}", str2.Replace("\"", "\\\""), ++unknow_idx);
  ...
  #endregion mysql enum/set
}

[Table(DisableSyncStructure = true)]
internal class TestTb
